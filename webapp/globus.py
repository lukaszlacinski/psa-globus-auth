...
class GlobusOpenIdConnect(GlobusOpenIdConnectBase):
    ...
    def get_user_details(self, response):
        ...
        if not self.setting('SESSIONS'):
            return super(GlobusOpenIdConnect, self).get_user_details(response)
        ...
        resp = self.get_json(
            ...
            params={'ids': ','.join(identities_set), 'include': 'identity_provider'},
        )

        for item in resp.get('identities'):
            if item.get('id') == identity_id:
                fullname, first_name, last_name = self.get_user_names(item.get('name'))
                return {
    ...
    def auth_allowed(self, response, details):
        ...
        for identity_id, identity_property in identity_set_properties.items():
            if identity_property.get('status') == 'active':
                raise AuthForbidden(
                    self,
                    {'session_message': 'To gain access you need to authenticate with your identity with an active membership status in the {} group.'.format(group_name),
        ...
        raise AuthForbidden(self, {'group_name': group_name, 'group_join_url': group_join_url})

    def auth_params(self, state=None):
        params = super(GlobusOpenIdConnect, self).auth_params(state)

        # If Globus sessions are enabled, force Globus login, and specify a required identity if it is known
        if not self.setting('SESSIONS'):
            return params
        params['prompt'] = 'login'
        ...
        if session_message:
            ...
            params['session_required_identities'] = self.strategy.session_pop('session_required_identities')
        return params
